...
class DPAModelEvaluator:
    ...
    def evaluate_model(self, name: str) -> None:
        ...
        mc_plot.show(
            ...
            output_dir,
            f'matriz_confusao_{confusion_matrix_file[name]}_dpa.png'))
        plt.close()
    ...
    def save_best_model(
            self, output_path: str = 'classificador_ml_dpa.pkl') -> None:
...
if __name__ == "__main__":
    ...
    data_dpa = PipelineDPA('data/barragens.csv')
    evaluator_dpa = DPAModelEvaluator(
        data_dpa.run_full_dpa_pipeline('data/barragens.csv'), target_index=0)
    
    evaluator_dpa.split_data(test_size=0.3, random_state=0)
    
    # Example usage with models (assuming models are defined elsewhere)
    evaluator_dpa.add_model(DecisionTreeClassifier(), 'DecisionTree')
    evaluator_dpa.add_model(RandomForestClassifier(), 'RandomForest')
    evaluator_dpa.add_model(KNeighborsClassifier(), 'KNN')
    evaluator_dpa.add_model(GaussianNB(), 'NaiveBayes')
    evaluator_dpa.add_model(SVC(), 'SVM')

    evaluator_dpa.train_all_models()
    for model_name in evaluator_dpa.models.keys():
        evaluator_dpa.evaluate_model(model_name)
    
    evaluator_dpa.save_best_model()